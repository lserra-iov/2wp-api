...
export class PeginStatusService {
    ...
    return this.getBtcInfo(btcTxId)
      .then(btcStatus => {
        ...
        if (btcStatus.requiredConfirmation <= btcStatus.confirmations) {
          return this.getRskInfo(btcStatus.btcWTxId).then(rskStatus => {
            if (rskStatus.status !== undefined) {
              peginStatusInfo.setRskPeginStatus(rskStatus);
              this.logger.debug(
                `Tx: ${btcTxId} includes rsk info. RskAddress: ${rskStatus.recipientAddress} Pegin status: ${peginStatusInfo.status}`,
              );
              return peginStatusInfo;
            } else {
              ...
              this.logger.debug(
                `Tx: ${btcTxId} not in RSK yet. Pegin status: ${peginStatusInfo.status}`,
              );
          ...
          this.logger.debug(
            `Tx: ${btcTxId} waiting confirmations. Pegin status: ${peginStatusInfo.status}`,
          );
          peginStatusInfo.setRskPeginStatus(peginRskInfo);
          return peginStatusInfo;
        }
      ...
      .catch(e => {
        this.logger.debug(
          `TxId:${btcTxId} Unexpected error trying to obtain information. Error: ${e}`,
        );
  ...
  private getBtcTxInfoFromService(btcTxId: string): Promise<BtcPeginStatus> {
    return this.bitcoinService.getTx(btcTxId).then(async (btcTx: BitcoinTx) => {
      ...
      if (!this.canBeAPeginSender(btcTx)) {
        ...
        const federationAddress =
          await this.bridgeService.getFederationAddress();
        if (!this.isSentToFederationAddress(federationAddress, btcTx.vout)) {
          ...
          btcStatus.amountTransferred = this.fromSatoshiToBtc(
            this.getTxSentAmountByAddress(
              federationAddress,
              btcTxId,
              btcTx.vout,
            ),
          ...
          btcStatus.requiredConfirmation =
            Number(process.env.BTC_CONFIRMATIONS) ?? 100;
  ...
  private getRskInfo(btcTxId: string): Promise<RskPeginStatus> {
    ...
    return this.rskDataService
      .getById(ensure0x(btcTxId))
      .then(async rskData => {
        if (rskData) {
  ...
  private isSentToFederationAddress(
  ...
  private getTxSentAmountByAddress(
    federationAddress: string,
    txId: string,
    vout: Vout[],
  ): number {
  ...
  private getxDestinationRskAddress(btcTx: BitcoinTx): string {
    ...
    for (
      let i = 0;
      btcTx.vout && i < btcTx.vout.length && !foundOpReturn;
      i++
    ) {
  ...
  private hasRefundOpReturn(txId: string, data: string): boolean {
    if (this.hasOpReturn(txId, data)) {
      // Includes version 01 in the same if
      if (data.length == 96) {
        //Contain refund address
        return true;
      }
    }
    return false;
  ...
  private hasOpReturn(txId: string, data: string): boolean {
    if (data.startsWith('6a') && data.substr(4, 10).startsWith('52534b5401')) {
      // Includes version 01 in the same if
      if (data.length == 96 || data.length == 54) {
        //Contain refund address
  ...
  private canBeAPeginSender(btcTx: BitcoinTx): boolean {
    try {
      if (
        peginAddressVerifier.canPegIn(
          peginAddressVerifier.getAddressInformation(btcTx.vin[0].addresses[0]),
        )
      ) {
        return true;
