    ...
    return Promise.all(addressInfoPromises)
      .then((bitcoinAddressList: BitcoinAddress[]) => {
        for (const bitcoinAddress of bitcoinAddressList) {
          const totalReceived: SatoshiBig = new SatoshiBig(
            bitcoinAddress.totalReceived,
            'satoshi',
          );
          const totalSent: SatoshiBig = new SatoshiBig(
            bitcoinAddress.totalSent,
            'satoshi',
          );
          ...
          this.logger.trace(
            `[isUnusedAddresses] ${bitcoinAddress} is ${
              unused ? 'unused' : 'used'
            }`,
          );
        }
