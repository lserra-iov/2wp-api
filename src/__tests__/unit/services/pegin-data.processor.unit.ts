...
const txHash =
  '0x604aaf3de25d0ab07c209b564cf1a4e7084e8750eaef23bf89966a1d2e7f19ad';

const getFakeRegisterBtcTransactionData = () => {
  const rawTx = ensure0x(
    '',
  );
  return remove0x(
...
describe('Service: PeginDataProcessor', () => {
  ...
  it('parses a transaction with a PEGIN_BTC log properly', () => {
    ...
    tx.data =
      ...
      getFakeRegisterBtcTransactionData();
    const log = new Log();
  ...
  it('parses a transaction with REJECTED_PEGIN and RELEASE_REQUESTED logs as a rejected pegin with refund', () => {
    ...
    const log = new Log();
    log.topics = [
      getBridgeSignature(BRIDGE_EVENTS.REJECTED_PEGIN),
      getBridgeSignature(BRIDGE_EVENTS.RELEASE_REQUESTED),
    ];
    tx.logs.push(log);
    const thisService = new PeginDataProcessor(mockedPeginStatusDataService);
    const result = thisService.parse(tx);
  ...
  it('parses a transaction with REJECTED_PEGIN and UNREFUNDABLE_PEGIN logs as a rejected pegin with no refund', () => {
    ...
    log.topics = [
      getBridgeSignature(BRIDGE_EVENTS.REJECTED_PEGIN),
      getBridgeSignature(BRIDGE_EVENTS.UNREFUNDABLE_PEGIN),
    ];
    tx.logs.push(log);
    const thisService = new PeginDataProcessor(mockedPeginStatusDataService);
    const result = thisService.parse(tx);
  ...
  it('processes pegin transaction', async () => {
    ...
    ) as SinonStubbedInstance<PeginStatusDataService>;
    const tx = new RskTransaction();
    tx.data =
      getBridgeSignature(BRIDGE_METHODS.REGISTER_BTC_TRANSACTION) +
      getFakeRegisterBtcTransactionData();
