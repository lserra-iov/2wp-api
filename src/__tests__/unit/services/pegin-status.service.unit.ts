...
const getBitcoinTxWithOpReturn = (
  btcTxId: string,
  from: string,
  amount: number,
  confirmations: number,
  ...
  opReturnValue.hex =
    '6a2e52534b5401224d0b72bab9342f898c633ef187abff8a96c0fa014a74c48b9e3a5644adb734ab536cab6ae28e85ce';
...
const getPeginStatusServiceWithMockedEnvironment = (
  ...
  const mockedBridgeService = sinon.createStubInstance(
    BridgeService,
  ...
  const mockedPeginStatusMongoDbDataService = sinon.createStubInstance(
...
describe('function: getPeginSatusInfo', () => {
  ...
  it('getPeginSatusInfo txId has an unexpected error', async () => {
    ...
    const thisService = getPeginStatusServiceWithMockedEnvironment(
      undefined,
      5,
    );
  ...
  it('getPeginSatusInfo bech32 with OP_RETURNS return NOT_IN_RSK_YET', async () => {
    ...
    expect(result.btc.federationAddress).to.be.equal(federationAddress);
    expect(result.btc.confirmations).to.be.equal(200);
    expect(result.rsk.recipientAddress).to.be.equal(
      '0x224d0b72bab9342f898c633ef187abff8a96c0fa',
    );
    expect(result.status).equal(Status.NOT_IN_RSK_YET);
  });
