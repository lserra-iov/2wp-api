...
describe('tx Fee controller', () => {
  ...
  it('should store a optimal input list given based on fastFee amount', async () => {
    findAccountUtxos
      .withArgs(sessionId, constants.BITCOIN_LEGACY_ADDRESS)
      .resolves(utxos1);
    await txFeeController.getTxFee(
      new FeeRequestData({
        sessionId,
        amount: 97410,
  ...
  it('should return the fee of the required amount even if there are no enough balance, and should be the max fee given all utxos', async () => {
    findAccountUtxos
      ...
      .resolves(utxos1);
    const amount = 393001;
    const fees = await txFeeController.getTxFee(
      new FeeRequestData({
        sessionId,
        amount,
        accountType: constants.BITCOIN_LEGACY_ADDRESS,
    ...
    const totalBytes: Big = new Big(
      (utxos1.length * +inputSize + txBytes).toString(),
    );
    expect(setInputs.notCalled).to.be.true();
    expect(fees).to.be.eql(
      new FeeAmountData({
        slow: Number(totalBytes.mul(lowAmount.div(1000)).toString()),
        average: Number(totalBytes.mul(averageAmount.div(1000)).toString()),
        fast: Number(totalBytes.mul(fastAmount.div(1000)).toString()),
  ...
  it(`should ensure the fee amount are at least ${constants.BITCOIN_MIN_SATOSHI_FEE} satoshis `, async () => {
    ...
    expect(fees.fast).to.be.greaterThanOrEqual(
      constants.BITCOIN_MIN_SATOSHI_FEE,
    );
    expect(fees.average).to.be.greaterThanOrEqual(
      constants.BITCOIN_MIN_SATOSHI_FEE,
    );
    expect(fees.slow).to.be.greaterThanOrEqual(
      constants.BITCOIN_MIN_SATOSHI_FEE,
    );
  });
  ...
  it('should ensure the mininum Fee Calculated were based on the environment varibles given', async () => {
    const minFastFee = new SatoshiBig(
      process.env.FEE_PER_KB_FAST_MIN ?? 100,
      'satoshi',
    );
    const minAverageFee = new SatoshiBig(
      process.env.FEE_PER_KB_AVERAGE_MIN ?? 100,
      'satoshi',
    );
    const minSlowFee = new SatoshiBig(
      process.env.FEE_PER_KB_SLOW_MIN ?? 100,
      'satoshi',
    );
  ...
  it('Should ensure the change output has a higher value than dust environment variable ', async () => {
    ...
    const totalBytes: Big = new Big(
      (2 * +inputSize + 3 * outputSize + txHeaderSize).toString(),
    );
    const changeAmount =
      200000 - amount - totalBytes.mul(minFastFee).toNumber();
    expect(
      setInputs.calledOnceWith(
        sessionId,
        [
