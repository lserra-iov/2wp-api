...
describe('function: getPeginSatusInfo', () => {
  ...
  it('getRefundAddress P2SH valid', async () => {
    ...
    const result = utility.getRefundAddress(
      '02379ad9b7ba73bdc1e29e286e014d4e2e1f6884e3',
    );
  ...
  it('getRefundAddress P2PKH valid', async () => {
    const utility = new BtcAddressUtils();
    const result = utility.getRefundAddress(
      '01ccc198c15d8344c73da67a75509a85a8f4226636',
    );
  ...
  it('getRefundAddress type invalid', async () => {
    const utility = new BtcAddressUtils();
    const result = utility.getRefundAddress(
      '05379ad9b7ba73bdc1e29e286e014d4e2e1f6884e3',
    );
    expect(result).to.be.empty();
  });
  ...
  it('calculateBtcTxHash calculate tx hash with segwit ', async () => {
    ...
    expect(result).to.be.equal(
      'cfca48c97bb00c524837149b71df3ad908ba44ee61244efc8fe2b13953d58105',
    );
  });
  ...
  it('calculateBtcTxHash calculate tx hash without segwit ', async () => {
    ...
    expect(result).to.be.equal(
      '1722f68a17b151894cb62b7b2971e62e16412feb57ca714d231d686d1a661f96',
    );
  });

  it('Validate the account type given an address', () => {
    getLegacyAddressList().forEach(walletAddress => {
      const {addressType} = btcAddressUtils.validateAddress(
        walletAddress.address,
      );
      expect(addressType).to.eql(constants.BITCOIN_LEGACY_ADDRESS);
    });
    getNativeSegwitAddressList().forEach(walletAddress => {
      const {addressType} = btcAddressUtils.validateAddress(
        walletAddress.address,
      );
      expect(addressType).to.eql(constants.BITCOIN_NATIVE_SEGWIT_ADDRESS);
    });
    getSewitAddressList().forEach(walletAddress => {
      const {addressType} = btcAddressUtils.validateAddress(
        walletAddress.address,
      );
      expect(addressType).to.eql(constants.BITCOIN_SEGWIT_ADDRESS);
    });
  });
});
