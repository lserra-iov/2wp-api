...
export class BalanceController {
  ...
  async getBalance(
    ...
    this.logger.debug(`[getBalance] Started with getBalance ${getBalance.sessionId}`);
    return new Promise<AccountBalance>((resolve, reject) => {
      ...
      Promise.all(eventualUtxos)
        ...
        .then(([result, addressBalances]) => {
          ...
          this.logger.debug(`[getBalance] accBalance ${JSON.stringify(accBalance)}`);
          resolve(accBalance);
        })
        .catch(reason => {
          this.logger.error(`[getBalance] Something went wrong. error: `, reason);
          reject(reason);
        });
    });
  }
